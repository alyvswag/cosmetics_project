...
class CatalogServiceImplTest {
    ...
    void getHomeCatalog_Success() {
        ...
        List<Products> arrivalsList = new ArrayList<>(List.of(p1));
        List<Products> bestSellersList = new ArrayList<>(List.of(p2));

        Page<Products> arrivalsPage = mock(Page.class);
        Page<Products> bestSellersPage = mock(Page.class);

        when(arrivalsPage.getContent()).thenReturn(arrivalsList);
        when(bestSellersPage.getContent()).thenReturn(bestSellersList);
